...
JsSummarize.prototype.summarize = function (title, text) {
	...
	//Filtering sentence
	var index;
	for (index = 0;index<sentences[0].length;index++)
	{
		if(isalpha(sentences[0].charAt(index)) && sentences[0].charAt(index+1) === " ")
			break; 
	}
	sentences[0] = sentences[0].slice(index+2,sentences[0].length)
	//document.write(sentences);

	var unique_array = []
    for(var i = 0;i < sentences.length; i++){
		sentences[i] = sentences[i].trim();
        if(unique_array.indexOf(sentences[i]) == -1){
            unique_array.push(sentences[i])
        }
    }
	//document.write(unique_array);

    var scoredSentences = this.score(unique_array, titleWords, keywords);	

	var returnlength = Math.ceil(unique_array.length*0.2);
	//document.write("The return length and sentence length ",returnlength,"  ",unique_array.length);
	...
	//Generating sentence in perfect order
	var sentenceOrder = [];
	for (var i = 0;i<orderedList.length;i++)
	{
		sentenceOrder.push({sentence:orderedList[i],index:sentences.indexOf(orderedList[i])});
	
	}
	var finalsentences = _.chain(sentenceOrder).sortBy("index").pluck("sentence").value();
